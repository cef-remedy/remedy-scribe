...
def stream_object_range(key: str, range_header: str | None) -> tuple[bytes, int, int | None, int | None]:
    """Range-aware read, present only so the interface is total.

    Nothing calls this on the S3 path: a presigned GET serves ranges
    directly from object storage and the API never sees the audio, which
    is the whole point of decision 0013. It exists because the Drive
    backend must proxy playback, and a dispatcher with a hole in it is
    worse than one method that is never reached.
    """
    kwargs: dict[str, Any] = {"Bucket": get_settings().s3_bucket, "Key": key}
    if range_header:
    ...
    if content_range := response.get("ContentRange"):
        ...
        total = int(response.get("ContentLength", len(body)))

    return body, 206 if range_header else 200, len(body), total
