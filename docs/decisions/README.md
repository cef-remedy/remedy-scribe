...
| [0031](0031-phi-encryption-stays-in-the-application-layer.md) | 4.1 | **App-layer Fernet stays**: `pgcrypto` puts the key in the SQL statement (and the query log) and does not solve rotation either; KMS envelope is the long-term answer. Rotation rehearsed for real | Decided (implementation) — **181 s over 1.73 GB; crypto is only 15% of it. Custody left to the DPO** |
| [0032](0032-audit-every-disclosure-and-make-the-log-undeletable.md) | 4.2 | One rule instead of 23 judgment calls: log every *disclosure of, or capability over*, PHI; never log requests. Append-only **plus a TRUNCATE guard**, and a retention period longer than the PHI it describes | Decided (implementation) — **`PATCH /notes` was writing no audit row at all** |
| [0033](0033-retention-is-enforced-in-two-layers.md) | 4.4 | Bucket lifecycle as the storage-layer backstop **and** a Celery Beat job for the derived rows Postgres holds — resolving a 🧠 the checklist flagged, without touching the part that is genuinely the user's (how long) | Decided (implementation) — **revisions must die with the transcript or grounding lies** |
| [0034](0034-an-untested-control-is-a-hope.md) | 4.3 | argon2id for new credentials, bcrypt verify-only for old, passlib removed — deleting the *reason* for the backwards pin rather than carrying it forward | Decided (implementation) — **measured: the pin was three versions early** |
| [0035](0035-groq-note-generation.md) | 4 | Note generation moves to Groq (`openai/gpt-oss-120b`), forced tool use — strict schema-constrained decoding. **The free tier cannot carry a consultation and is excluded from the BAA**; the real argument is one processor instead of two | Decided (user + implementation) — **the stated reason did not survive checking; the change is still right** |
| [0036](0036-one-vm-and-one-bought-database.md) | 5.1 | One Linux VM on Docker Compose; Postgres is the single bought service (PITR is the hard part), Redis self-hosted with the loss window quantified. Kubernetes rejected: a control plane does not move where data lives | Decided (implementation) — **jurisdiction and provider left to Legal, as four ordered questions** |
| [0037](0037-logs-are-assembled-from-an-allow-list-not-scrubbed-after.md) | 5.2 | Log records are rebuilt from their template and assembled from an allow-list, at a boundary no call site can route around — a denylist someone must remember to update is the failure mode | Decided (implementation) — **Sentry ships stack-frame locals by default, and a frame in `generate_note` holds the transcript** |
| [0038](0038-ci-gates-what-a-green-check-is-allowed-to-mean.md) | 5.3 | What a green check is allowed to mean: a migration gate as a **snapshot assertion, not an ignore list** (it fails when a divergence disappears too), Postgres suites that cannot silently skip, Playwright kept out on purpose | Decided (implementation) — **found four real divergences nothing had ever compared** |
| [0039](0039-a-minor-edit-is-small-and-clinically-inert.md) | 6 | A "minor edit" is **small AND clinically inert** — a clinical veto, not a weighting, since `500mg`—`5000mg` is one character and a 10x overdose. Measured generated—signed, chain-rooted without timestamps, frozen with its definition version | Decided (implementation) — **the 2.6 🧠, deferred by design and now answered** |
| [0040](0040-google-drive-as-a-storage-backend.md) | deploy | Google Drive as an audio backend behind a dispatcher. Uploads stay browser-direct (**CORS verified empirically, Google documents none of it**); playback cannot, so it proxies through the API. **No service account is possible on a free account** — the audio is owned by a named human | Decided (user + implementation) — **three real costs, each stated rather than absorbed** |
| [0041](0041-upload-parts-must-align-to-exact-byte-boundaries.md) | post-deploy | Upload parts are cut on exact byte boundaries, not grouped by whole recorder chunks — Drive needs an exact multiple of its 256 KiB floor, not merely "at least" | Decided (implementation) — **found live: no consultation over ~a minute could upload on the Drive backend, deterministically, through all 8 retries** |
