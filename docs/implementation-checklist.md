...
**Absent entirely:** the grounding UI's data path (Phase 3); retention *enforcement* (the columns exist, no job reads them); **every doctor-facing feature above the auth shell** — recording (2.2), consent flow (2.3), offline upload queue (2.4), patient identity (2.5), note review and signing (2.6); and — a genuine, currently-open gap against a written requirement, not an oversight — speaker diarization (P0-3), which the ASR vendor in use structurally cannot provide.

The honest headline for the doctor-facing side has changed shape but not conclusion: **a doctor still cannot use this.** There is now a real client — it signs in, holds a session across reloads, and reads its worklist from the API through a generated typed client — but it cannot yet record, capture consent, queue an upload, find a patient, or sign a note. Phase 0 and all of Phase 1 are closed, and Phase 2.1 is the app's foundation rather than its function. Next is **2.2 (recording)**, which is where the capture harness's findings get used for real, and **2.3 (consent)**, which must gate recording before any of it touches a patient (P0-1).
