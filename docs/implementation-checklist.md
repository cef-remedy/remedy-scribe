...
**Absent entirely:** the grounding UI's data path (Phase 3); retention *enforcement* (the columns exist, no job reads them); **the consent flow (2.3), which is the thing recording is gated behind — so recording exists but cannot legally be started from the app yet**; the offline upload queue (2.4), so recorded chunks accumulate locally and are never sent or deleted; patient identity (2.5); note review and signing (2.6); and — a genuine, currently-open gap against a written requirement, not an oversight — speaker diarization (P0-3), which the ASR vendor in use structurally cannot provide.

The honest headline: **a doctor still cannot use this, and the reason is now precise rather than broad.** The client signs in, holds a session across reloads, and records real encrypted audio with gap detection — but the record button is unreachable, by design, because the consent flow that would unlock it does not exist. That is 2.3, and it is the single next thing standing between this and a first supervised trial. After it, 2.4 (upload queue) is required before anything recorded can leave the laptop — today chunks accumulate in IndexedDB and are never sent, so a clinic laptop would eventually fill up. Phase 0 and all of Phase 1 are closed; Phase 2 is 2 of 6 subphases in.
...
- [ ] Durable queue surviving app kill and device restart — **IndexedDB**, not `expo-sqlite` (decision 0024). Same write-ahead-log invariant, different store.
