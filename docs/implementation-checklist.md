...
**Absent entirely:** pilot instrumentation (Phase 6), including the edit-burden metric that decides whether the pilot passes; deployment and operations (Phase 5), which still owes TLS termination and the secret manager Phase 4.3 could only design; **detection** — nothing watches the retention sweep or alerts on anything, and the NPC breach clock starts at *reasonable belief*, so detection rather than notification is the binding constraint; a patient **merge** tool; a designated **DPO and breach response team**, both legally required and currently nonexistent; and — a genuine, currently-open gap against a written requirement, not an oversight — speaker diarization (P0-3), which the ASR vendor in use structurally cannot provide.

The honest headline: **the product does the thing it exists to do, it can be checked while doing it, and it is now defensible enough to argue about with a regulator.** A doctor captures consent, records, gets a draft note, clicks any line to see and hear where it came from, edits, files against a re-confirmed identity, and signs. Underneath that: PHI keys that can actually be rotated (rehearsed, 181 seconds), an audit trail that records every disclosure and cannot be quietly emptied, a retention job that finally deletes what two columns have been marking as expired since Phase 1, and dependency scanning that took 30 advisories to 1.

**What Phase 4 mostly did was find things that were already wrong.** A PHI leak into an unencrypted column. A clinical-content edit that wrote no audit row. A `TRUNCATE` that would have emptied the audit log while leaving "append-only" technically true. A `bcrypt` pin held backwards for years on the strength of a log line, three versions earlier than the break it was avoiding. None of these were in the new code, and none would have surfaced without going looking.

Three things stand between this and a supervised pilot, and only one is engineering:

1. **Legal and the DPO have real work waiting**, and it is now specific rather than vague: clear the RA 4200 consent script; decide PHI-at-rest custody (HSM/KMS, and whether deploy access should keep equalling PHI access — today it does); confirm the breach runbook's notification obligations, which were assembled from secondary sources because the NPC site refused direct fetching; set the retention period; and **designate a DPO and a breach response team, both legally required and both currently nonexistent**.
2. **A paid Groq plan.** The free tier cannot carry a consultation transcript and is excluded from the BAA. This is a purchasing decision, not an engineering one, but nothing should touch real PHI until it is made.
3. **Pilot instrumentation (Phase 6)** — defining and computing edit burden, the metric that decides whether the pilot passes.

And one thing that is nobody's blocker but should not be a surprise: **nothing watches anything yet.** There is no alerting, and the NPC breach clock starts at *reasonable belief*, which makes detection — not notification — the binding constraint. That is Phase 5.2's, and it is the gap that would make a good runbook useless.

Phase 0, Phase 1, Phase 2, Phase 3 and Phase 4 are all closed.
