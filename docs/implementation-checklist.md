...
The honest headline: **the software is finished for a pilot; the things that remain are an account, a signature and a server.** A doctor captures consent, records, gets a draft note, clicks any line to see and hear where it came from, edits, files against a re-confirmed identity, and signs. Underneath it: rotatable PHI keys, an audit trail that cannot be quietly emptied, a retention job that actually deletes, dependency scanning, a migration gate that proves the schema matches its models, and health checks that distinguish "restart me" from "don't send me traffic".

**Every hardening phase has paid for itself in defects nobody was looking for.** Phase 4 found a PHI leak into an unencrypted column, a clinical edit that wrote no audit row, and a `TRUNCATE` that would have emptied the audit log. Phase 5 found `apps/api/.env` being baked into the Docker image with a live key — which Phase 4.1's boot guard would *not* have caught, because that key is not the published dev secret — a login rate limit that would have silently become per-clinic behind a proxy, and four schema divergences nothing had ever compared. None were in new code.

What stands between this and a supervised pilot is now almost entirely **not engineering**:

1. **Legal and the DPO.** Clear the RA 4200 consent script. Answer decision 0036's four ordered questions on data residency — including whether Groq already holding the audio and transcript settles the processor-boundary question. Confirm the breach runbook's obligations, which were assembled from secondary sources because the NPC site refused direct fetching. And **designate a DPO and a breach response team, both legally required and both currently nonexistent.**
2. **Three accounts and a server.** A paid Groq plan (the free tier cannot carry a consultation and is excluded from the BAA), a Sentry account (without which every alert rule written in 5.2 is an ERROR line in a file nobody reads), a managed Postgres, and a VM. Nothing is deployed; no CI job has run on a real runner.
3. **Pilot instrumentation (Phase 6)** — defining and computing edit burden, the metric that decides whether the pilot passes. The only remaining engineering phase.

And one number worth carrying into the pilot design: the PRD's **<$0.10/consult** target breaks even at **51.7 minutes of audio**, because ASR costs 15–23× note generation. That target is really a *duration* budget, and no prompt tuning will move it.

Phase 0 through Phase 5 are all closed.
