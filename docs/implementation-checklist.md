...
**Absent entirely:** the grounding UI's data path (Phase 3); retention *enforcement* (the columns exist, no job reads them); the grounding UI (Phase 3), so a doctor cannot yet tap a note line and hear where it came from — the product's trust mechanism; pilot instrumentation (Phase 6), including the edit-burden metric that decides whether the pilot passes; and — a genuine, currently-open gap against a written requirement, not an oversight — speaker diarization (P0-3), which the ASR vendor in use structurally cannot provide.

The honest headline: **Phase 2 is complete and a doctor can now go from consent to signature in one pass** — capture consent bilingually, record encrypted audio, have it upload and transcribe, find the patient by name, review the draft in APSO order, edit it, file it against a re-confirmed identity, and sign it under their PRC licence, after which it is immutable. Tested as one flow, against the real stack.

Three things still stand between this and a supervised pilot, and only two are engineering:
...
2. **The grounding UI (Phase 3)** — tap a note line, see and hear the passage it came from. This is the product's trust mechanism: the doctor's rational response to "an AI wrote this" is "prove it", and today there is no way to.
3. **Pilot instrumentation (Phase 6)**, which includes defining and computing edit burden — the metric that decides whether the pilot passes. The checklist is right that instrumentation must land *before* alpha, not after, or the accepted vendor risk becomes an unmonitored one.

Phase 0, Phase 1 and Phase 2 are all closed.
