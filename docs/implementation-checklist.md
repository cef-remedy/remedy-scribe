...
That last line is still the honest headline for the doctor-facing side: **the mobile app does not exist yet.** The server side has moved substantially since the last "current state" was written — Phase 0 and all of Phase 1 (1.1 through 1.5) are now fully closed — but nothing here is usable by an actual doctor until Phase 2 exists. The suggested sequencing table's next step is Phase 2.1–2.2: mobile app foundation and recording, the longest-lead, highest-risk work still ahead.
