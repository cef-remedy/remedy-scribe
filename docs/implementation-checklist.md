...
That last line is still the honest headline for the doctor-facing side: **the mobile app does not exist yet.** The server side has moved substantially since the last "current state" was written — Phase 0 is fully closed, and 1.1–1.4 of Phase 1 are done, leaving only 1.5 (pipeline failure handling) before Phase 1 is complete — but nothing here is usable by an actual doctor until Phase 2 exists.
