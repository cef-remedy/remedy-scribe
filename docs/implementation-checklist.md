...
**Absent entirely:** the grounding UI's data path (Phase 3); retention *enforcement* (the columns exist, no job reads them); pilot instrumentation (Phase 6), including the edit-burden metric that decides whether the pilot passes; retention *enforcement*, so nothing yet deletes what the columns mark as expired (Phase 4.4); a patient **merge** tool; and — a genuine, currently-open gap against a written requirement, not an oversight — speaker diarization (P0-3), which the ASR vendor in use structurally cannot provide.

The honest headline: **the product now does the thing it exists to do, and can be checked while doing it.** A doctor captures consent bilingually, records, and gets a draft note; then clicks any line of it to see the transcript passage it came from and hear that moment of the consultation; edits it, files it against a re-confirmed patient identity, and signs it under their PRC licence, after which it is immutable. Tested as one flow against the real stack.

Grounding is what turns the rest of it from a tool a doctor has to trust into one they can audit line by line — and it was built to fail *loudly* rather than plausibly. Where the source links can no longer be proven correct, they are withheld with a reason instead of approximated; where the recording is gone, the screen says which of the reasons applies rather than showing a control that does nothing.

Two things still stand between this and a supervised pilot, and only one is engineering:

1. **Legal must clear the RA 4200 consent script.** The PRD's own blocking open question. The mechanism is ready for counsel's text; nothing should be read to a real patient before then.
2. **Pilot instrumentation (Phase 6)**, which includes defining and computing edit burden — the metric that decides whether the pilot passes. The checklist is right that instrumentation must land *before* alpha, not after, or the accepted vendor risk becomes an unmonitored one.

Not blocking a pilot but worth stating: **nothing yet deletes anything.** Both retention columns are written and read by no job (Phase 4.4), so the only thing removing expired audio today is the object store's own lifecycle rule — which is exactly why Phase 3 verifies audio against storage rather than believing the database.

Phase 0, Phase 1, Phase 2 and Phase 3 are all closed.
